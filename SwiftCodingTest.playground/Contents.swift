...
/*:
 ...
 ### 🔹 문제 설명
 - a = "18446744073709551615", b = "287346502836570928366" -> 결과: "305793246910280479981"
...
func solution44(_ a: String, _ b: String) -> String {
    ...
    for i in 0..<maxLength {
        let digitA = i < arrA.count ? arrA[i] : 0
        let digitB = i < arrB.count ? arrB[i] : 0
...
/*:
 ## 설명
 - Array(a.reversed()).map { Int(String($0))! }
 : 1의 자리부터 더하기 위해 문자열을 뒤집고, 각 문자를 정수로 변환
 
 - result.reserveCapacity(...)
 : 결과 배열의 용량을 미리 확보해 큰 입력에서도 불필요한 재할당을 줄임
 
 - for i in 0..<maxLen
 : 두 수의 최대 자리수만큼 반복
 
 - digitA, digitB
 : 해당 인덱스에 자리가 없으면 0으로 간주해 길이가 달라도 더함
 
 - sum % 10, sum / 10
 : 현재 자리 숫자와 올림(carry)을 분리
 
 - 마지막에 carry가 남아 있으면 result에 추가한 뒤, 뒤집어서 문자열로 합침
 */
